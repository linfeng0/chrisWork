...
exports.calendars = (year, month) => {
    ...
    let curMonth = new Array(monthDaySum[month]);
    for (let i = 0; i < curMonth.length; i++) {
        curMonth[i] = monthNum[month] + "-" + ((i + 1 < 10) ? "0" : "") + (i + 1);
    }
    let preMonth = new Array(firstDay);
    ...
    let nextMonth = new Array((35 - curMonth.length - firstDay < 0) ? 42 - curMonth.length - firstDay : 35 - curMonth.length - firstDay);
